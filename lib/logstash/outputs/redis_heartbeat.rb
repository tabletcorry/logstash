...
class LogStash::Outputs::RedisHeartbeat < LogStash::Outputs::Base

  config_name "redis_heartbeat"
  ...
  def register
    ...
    @type_heartbeats = Hash.new
    @type_tag_heartbeats = Hash.new { |h, k| h[k] = Hash.new }
  ...
  def receive(event)
    ...
    if @batch
      now = Time.now.to_i
      @type_heartbeats[event.type] = now
      event.tags.each do |tag|
        @type_tag_heartbeats[event.type][tag] = now
      end
      process_pending
      return
    else
      ...
      #
      raise RuntimeError.new(
        "Must run in batch. DO NOT MERGE THIS UNTIL FIXED"
      )
    end
  ...
  def process_pending(force=false)
    ...
    if (force) ||
      ...
      begin
        @redis ||= connect
        @redis.hmset "logstash:redis_heartbeat:types", @type_heartbeats.flatten
        @type_tag_heartbeats.each do |k, v|
          @redis.hmset "logstash:redis_heartbeat:type:" << k, v.flatten
        end
        @last_pending_flush = Time.now.to_f
      rescue => e
